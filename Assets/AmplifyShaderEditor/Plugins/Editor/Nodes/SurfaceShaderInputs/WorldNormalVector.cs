		...
		public override string GenerateShaderForOutput( int outputId, ref MasterNodeDataCollector dataCollector, bool ignoreLocalvar )
		{
			if( dataCollector.PortCategory == MasterNodePortCategory.Fragment || dataCollector.PortCategory == MasterNodePortCategory.Debug )
			{
				...
				if ( m_inputPorts[ 0 ].IsConnected )
				{
					...
					{
						dataCollector.AddToLocalVariables( m_uniqueId, string.Format( NormalVecDecStr, NormalVecValStr + m_uniqueId, result ) );
						return GetOutputVectorItem( 0, outputId, NormalVecValStr + m_uniqueId );
					}
				}
				else
				{
					...
					{
						dataCollector.AddToInput( m_uniqueId, Constants.InternalData, false );
						result = GeneratorUtils.GenerateWorldNormal( ref dataCollector, m_uniqueId );
					}
				}
