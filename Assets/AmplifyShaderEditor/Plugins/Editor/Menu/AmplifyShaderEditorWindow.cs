...
namespace AmplifyShaderEditor
{
	public class AmplifyShaderEditorWindow : EditorWindow, ISerializationCallbackReceiver
	{
		...
		public void OnEnable()
		{
			...
			m_shortcutManager.RegisterEditorShortcut( true, KeyCode.Space, "Open Node Palette", null, () =>
			  {
		...
		ParentNode CreateNodeFromClipboardData( int clipId )
		{
			...
			if ( newNode != null )
			{
				try
				{
					newNode.FullReadFromString( ref parameters );
				}
				catch ( Exception e )
				{
			...
			if ( m_palettePopup.IsActive )
			{
				...
				if ( m_currentEvent.GetTypeForControl( controlID ) == EventType.MouseUp )
				{
					if ( m_currentEvent.button == ButtonClickId.LeftMouseButton )
					{
						...
						if ( !InsideMenus( m_currentMousePos2D ) )
						{
							ParentNode newNode = CreateNode( m_paletteChosenType, TranformedMousePos );
							m_mainGraphInstance.SelectNode( newNode, false, false );
						}
					}
				}
		...
		{
			m_mainGraphInstance.DeSelectAll();
			ParentNode newNode = CreateNode( type, m_contextPalette.StartDropPosition * m_cameraZoom - m_cameraOffset );
			m_mainGraphInstance.SelectNode( newNode, false, false );
		}
