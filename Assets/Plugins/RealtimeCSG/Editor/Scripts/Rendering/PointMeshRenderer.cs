...
namespace RealtimeCSG
{
	sealed class PointMeshManager
	{
		sealed class PointMesh
		{
			...
			public int[]		pointIndices	= new int    [MaxVertexCount];
			public Color[]		pointColors		= new Color  [MaxVertexCount];

			public int[]		lineIndices		= new int    [MaxVertexCount / 4 * 6];
			public Color[]		lineColors		= new Color  [MaxVertexCount / 4 * 8];
			...
			public void CommitMesh()
			{
				...
				pointMesh.SetIndices(newPointIndices, MeshTopology.Triangles, 0);
				pointMesh.UploadMeshData(true);
				
				lineMesh.vertices = newVertices;
				lineMesh.colors = newLineColors;
				lineMesh.SetIndices(newLineIndices, MeshTopology.Lines, 0);
		...
		public void DrawPoints(Vector3[] vertices, float[] sizes, Color[] colors)
		{
			...
			{
				var index				= dstVertexCount;
				if (pointMesh.VertexCount + 4 >= PointMesh.MaxVertexCount)
				{
					pointMesh.vertexCount     = dstVertexCount;
					pointMesh.pointIndexCount = dstPointIndexCount;
					pointMesh.lineIndexCount  = dstLineIndexCount;
