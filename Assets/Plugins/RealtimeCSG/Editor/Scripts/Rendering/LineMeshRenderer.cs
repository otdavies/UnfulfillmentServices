	...
	sealed class LineMeshManager
	{
		sealed class LineMesh
		{
			...
			public void CommitMesh()
			{
				...
				List<Vector3>	newVertices1;
				List<Vector3>	newVertices2;
				List<Vector4>	newOffsets;
				List<Color>		newColors;

				if (vertexCount == MaxVertexCount)
				{
					newVertices1 = vertices1.ToList();
					newVertices2 = vertices2.ToList();
					newOffsets	 = offsets  .ToList();
					newColors	 = colors   .ToList();
				} else
				{ 
					newVertices1 = vertices1.Take(vertexCount).ToList();
					newVertices2 = vertices2.Take(vertexCount).ToList();
					newOffsets   = offsets  .Take(vertexCount).ToList();
					newColors    = colors   .Take(vertexCount).ToList();
				}
		...
		public void DrawLines(Vector3[] vertices, int[] indices, Color color, float thickness = 1.0f, float dashSize = 0.0f) //2
		{
			...
			{  
				for (int i = 0; i < indices.Length; i += 2)
				{
					...
					if (vertexCount + 4 >= LineMesh.MaxVertexCount) {  lineMeshIndex++; if (lineMeshIndex >= lineMeshes.Count) lineMeshes.Add(new LineMesh()); lineMesh	= lineMeshes[lineMeshIndex]; vertexCount = lineMesh.vertexCount; }		
					var vertices1	= lineMesh.vertices1;
					var vertices2	= lineMesh.vertices2;
					var offsets		= lineMesh.offsets;
					var colors		= lineMesh.colors;
