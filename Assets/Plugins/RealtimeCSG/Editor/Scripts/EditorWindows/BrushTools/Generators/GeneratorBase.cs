...
namespace RealtimeCSG
{
	internal abstract class GeneratorBase : ScriptableObject//, IBrushGenerator
	{
		...
		public virtual void Init()
		{
			...
			originalGridRotation	= RealtimeCSG.Grid.ForcedGridRotation;
			originalGridCenter		= RealtimeCSG.Grid.ForcedGridCenter;
			originalForceGrid		= RealtimeCSG.Grid.ForceGrid;
		...
		{
			RealtimeCSG.Grid.ForcedGridRotation = originalGridRotation;
			RealtimeCSG.Grid.ForcedGridCenter	= originalGridCenter;
			RealtimeCSG.Grid.ForceGrid			= originalForceGrid;
		}
		...
		protected void CalculateWorldSpaceTangents()
		{
			...
			{
				hadForcedGrid = RealtimeCSG.Grid.ForceGrid;
			}
			if (hadForcedGrid)
			{
				RealtimeCSG.Grid.ForceGrid = false;
				RealtimeCSG.Grid.UpdateGridOrientation();
			}

			var gridSpaceNormal = RealtimeCSG.Grid.VectorToGridSpace(buildPlane.normal);
			...
			gridNormal   = RealtimeCSG.Grid.VectorFromGridSpace(gridSpaceNormal);
			gridTangent  = RealtimeCSG.Grid.VectorFromGridSpace(gridSpaceTangent);
			gridBinormal = RealtimeCSG.Grid.VectorFromGridSpace(gridSpaceBinormal);
			
			toGridMatrix		= RealtimeCSG.Grid.ToGridSpaceMatrix();
			fromGridMatrix		= RealtimeCSG.Grid.FromGridSpaceMatrix();
			toGridQuaternion	= RealtimeCSG.Grid.ToGridSpaceQuaternion();
			fromGridQuaternion	= RealtimeCSG.Grid.FromGridSpaceQuaternion();
			
			if (hadForcedGrid)
			{
				RealtimeCSG.Grid.ForceGrid = true;
				RealtimeCSG.Grid.UpdateGridOrientation();
			}
		}
