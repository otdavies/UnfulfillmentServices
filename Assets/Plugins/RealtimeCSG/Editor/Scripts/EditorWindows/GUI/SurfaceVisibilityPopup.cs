...
namespace RealtimeCSG
{
	...
	internal class SurfaceVisibilityPopup : UnityEditor.PopupWindowContent
	{
		...
		{ 
			...
			GUI.Toggle(drawPos, helperSurfaces == allVisibleSurfaces, s_Flags.onlyVisibleContent, GUIStyleUtility.Skin.menuItem);
			...
			EditorGUI.BeginChangeCheck();
			GUI.Toggle(drawPos, helperSurfaces == allHelperSurfaces, s_Flags.allHelperContent, GUIStyleUtility.Skin.menuItem);
			if (EditorGUI.EndChangeCheck())
			{
		...
		{
			EditorGUI.BeginChangeCheck();
			bool result = GUI.Toggle(rect, ((helperSurfaces & flag) != 0), toggleName, GUIStyleUtility.Skin.menuItem);
			if (EditorGUI.EndChangeCheck())
			{
		...
		public static void Button(Rect currentRect)
		{
            ...
            if (!EditorGUI.showMixedValue)
            {
				...
				if (helperSurfaces == 0) { buttonContent = s_Flags.nothingContent; style = GUIStyleUtility.Skin.redToolbarDropDown; } else
