	...
	internal static class ControlMeshUtility
	{
		...
		public static bool Validate(ControlMesh controlMesh, Shape shape)
		{
			...
			for (var i = 0; i < controlMesh.Edges.Length; i++)
			{
				...
				var index = Array.IndexOf(edgeIndices, i);
				if (index == -1)
				{
					if (!_outputToLog) return false;
				...
				if (edgeIndices[index] != i)
				{
					if (!_outputToLog) return false;
					var text = (i + ": polygon.edges[index] != i");
					if (PrevOutput.Add(text)) Debug.Log(text);
					fail = true;
					continue;
				}
			...
			var usedTexGens = new HashSet<int>();
//			var used_planes  = new HashSet<int>();
			for (var p = 0; p < controlMesh.Polygons.Length; p++)
			{
				...
				usedTexGens.Add(texGenIndex);
			}

			for (var t = 0; t < shape.TexGens.Length; t++)
			{
				if (usedTexGens.Contains(t))
					continue;
		...
		{
			Debug.Log("MergeVerticesOnEdges");
			if (controlMesh == null)
				return;
			...
			for (var i = removeEdges.Count - 1; i >= 0; i--)
			{
				...
				for (var p = 0; p < polygons.Length; p++)
				{
					...
					for (var e = edgeIndices.Length - 1; e >= 0; e--)
					{
						...
						ArrayUtility.RemoveAt(ref edgeIndices, e);
					}
					polygons[p].EdgeIndices = edgeIndices;
				}
		...
		{
			var fromIndex	= ArrayUtility.IndexOf(edges, fromValue);
			var toIndex		= ArrayUtility.IndexOf(edges, toValue);
			if (fromIndex == -1 ||
				toIndex == -1)
				return null;
			...
			if (fromIndex < toIndex)
			{
				...
				Array.Copy(edges, fromIndex, result, 0, length);
				return result;
			} else
			{
				var offsetA = (edges.Length - fromIndex);
				var offsetB = (toIndex + 1);
				var length	= offsetA + offsetB;
				var result	= new int[length + additionalStorage];
				if (offsetA > 0)
				{
					Array.Copy(edges, fromIndex, result, 0, offsetA);
				}
		...
		{
			//Debug.Log("RemoveControlPoints");

			/*
			var removeEdges = new HashSet<int>();
			var removePolygons = new HashSet<int>();
			var tested = new HashSet<int>();

			for (int i = sortedRemovePoints.Count - 1; i >= 0; i--)
			{
				var vertexIndex = sortedRemovePoints[i];
				var firstEdgeIndex = FirstEdgeWithPoint(controlMesh, vertexIndex);
				if (firstEdgeIndex == -1)
					continue;
				...
				do
				{
					if (!tested.Add(iterator)) // safer, in case we have errors in our graph
						break;
					removeEdges.Add(iterator);
					removeEdges.Add(controlMesh.GetTwinEdgeIndex(iterator));
					iterator = controlMesh.GetNextEdgeIndexAroundVertex(iterator);
				} while (iterator != firstEdgeIndex);
			}

			for (int p = 0; p < controlMesh.polygons.Length; p++)
			{
				var edge_indices = controlMesh.polygons[p].edgeIndices;
				var removedEdgeCount = 0;
				for (int i = 0; i < edge_indices.Length; i++)
				{
					var edgeIndex = edge_indices[i];
					if (removeEdges.Contains(edgeIndex))
						removedEdgeCount++;
				}
				if (edge_indices.Length == removedEdgeCount)
					removePolygons.Add(p);
			}

			var sortedRemovePolygons = removePolygons.ToList();
			sortedRemovePolygons.Sort();
			

			var sortedRemoveEdges = removeEdges.ToList();
			sortedRemoveEdges.Sort();

			var edgeIndexRemap = new int[controlMesh.edges.Length];
			int offset = 0;
			for (int e = 0; e < controlMesh.edges.Length; e++)
			{
				if (offset < sortedRemoveEdges.Count &&
					e == sortedRemoveEdges[offset])
				{
					offset++;
					edgeIndexRemap[e] = -1;
				} else
					edgeIndexRemap[e] = e - offset;
			}

			var vertexIndexRemap = new short[controlMesh.vertices.Length];
			offset = 0;
			for (int v = 0; v < controlMesh.vertices.Length; v++)
			{
				if (offset < sortedRemovePoints.Count &&
					v == sortedRemovePoints[offset])
				{
					offset++;
					vertexIndexRemap[v] = -1;
				} else
					vertexIndexRemap[v] = (short)(v - offset);
			}

			for (int e = 0; e < controlMesh.edges.Length; e++)
			{
				controlMesh.edges[e].twinIndex = edgeIndexRemap[controlMesh.edges[e].twinIndex];
				controlMesh.edges[e].vertexIndex = vertexIndexRemap[controlMesh.edges[e].vertexIndex];
			}

			for (int s = sortedRemovePolygons.Count - 1; s >= 0; s--)
			{
				var value = sortedRemovePolygons[s];
				ArrayUtility.RemoveAt(ref controlMesh.polygons, value);
				ArrayUtility.RemoveAt(ref shape.Surfaces, value);
			...
			for (int e = sortedRemoveEdges.Count - 1; e >= 0; e--)
			{
				var value = sortedRemoveEdges[e];
				ArrayUtility.RemoveAt(ref controlMesh.edges, value);
			}

			for (int v = sortedRemovePoints.Count - 1; v >= 0; v--)
			{
				var value = sortedRemovePoints[v];

				ArrayUtility.RemoveAt(ref controlMesh.vertices, value);
			}

			for (int p = 0; p < controlMesh.polygons.Length; p++)
			{
				var edge_indices = controlMesh.polygons[p].edgeIndices;
				for (int e = edge_indices.Length - 1; e >= 0; e--)
				{
					var edgeIndex = edgeIndexRemap[edge_indices[e]];
					if (edgeIndex == -1)
					{
						ArrayUtility.RemoveAt(ref edge_indices, e);
						controlMesh.polygons[p].edgeIndices = edge_indices;
						continue;
					}
					controlMesh.edges[edgeIndex].polygonIndex = (short)p;
				}
			}
			/*/
		...
		{
			//Debug.Log("RemoveControlPoint");
			//ArrayUtility.RemoveAt(ref controlMesh.points, vertexIndex);
		...
		private static int CreateTexGen(Shape shape)
		{
			...
			ArrayUtility.Add(ref shape.TexGenFlags,	TexGenFlags.None);
		...
		{
			//Debug.Log("RemoveUnusedMaterials");
			var polygons = controlMesh.Polygons;
			for (var p = 0; p < polygons.Length; p++)
			{
				if (polygons[p].TexGenIndex < 0 ||
					polygons[p].TexGenIndex >= shape.TexGens.Length)
				{
					polygons[p].TexGenIndex = CreateTexGen(shape);
				}
			}

			var usedTexGens	= new bool[shape.TexGens.Length];
			for (var p = 0; p < polygons.Length; p++)
			{							
				usedTexGens[polygons[p].TexGenIndex] = true;
			}

			var indexConversion = new int[shape.TexGens.Length];

			var materialCount = usedTexGens.Length;
			for(var i = 0; i < materialCount; i++)
			{
				...
				indexConversion[found] = i;
				
				shape.TexGenFlags[i] = shape.TexGenFlags[found];
				shape.TexGens[i] = shape.TexGens[found];
				
				usedTexGens[i] = true;
				usedTexGens[found] = false;
				materialCount--;
			}

			if (materialCount == usedTexGens.Length)
				return;

			for (var i = 0; i < controlMesh.Polygons.Length; i++)
			{
				var texGenIndex = controlMesh.Polygons[i].TexGenIndex;
				controlMesh.Polygons[i].TexGenIndex = indexConversion[texGenIndex];
			}

			for (var i = usedTexGens.Length - 1; i >= 0; i--)
			{
				if (usedTexGens[i])
					break;
		...
		public static void FixTexGens(ControlMesh controlMesh, Shape shape)
		{
			...
			for (var p = 0; p < controlMesh.Polygons.Length; p++)
			{
				if (controlMesh.Polygons[p].TexGenIndex < 0 ||
					controlMesh.Polygons[p].TexGenIndex >= shape.TexGens.Length)
				{
					controlMesh.Polygons[p].TexGenIndex = CreateTexGen(shape);
				} 
				usedTexGens.Add(controlMesh.Polygons[p].TexGenIndex);
			}
...
//					var edge_index_count = polygon.edgeIndices.Length;
				...
				//Debug.Log("intersection_count: " + intersection_count);
				if (intersectionCount != 4)
				{
					//Debug.Log("Expected exactly 4 edge-plane traversals, found " + intersection_count);
					return false;
				}
				...
				//Debug.Log("travesal0[0] " + intersections[0].traversal0.edgeIndex + " " + intersections[0].traversal0.side.halfspace);
				//Debug.Log("travesal1[0] " + intersections[0].traversal1.edgeIndex + " " + intersections[0].traversal1.side.halfspace);
				//Debug.Log("travesal0[1] " + intersections[1].traversal0.edgeIndex + " " + intersections[1].traversal0.side.halfspace);
				//Debug.Log("travesal1[1] " + intersections[1].traversal1.edgeIndex + " " + intersections[1].traversal1.side.halfspace);
				//Debug.Log("travesal0[2] " + intersections[2].traversal0.edgeIndex + " " + intersections[2].traversal0.side.halfspace);
				//Debug.Log("travesal1[2] " + intersections[2].traversal1.edgeIndex + " " + intersections[2].traversal1.side.halfspace);
				//Debug.Log("travesal0[3] " + intersections[3].traversal0.edgeIndex + " " + intersections[3].traversal0.side.halfspace);
				//Debug.Log("travesal1[3] " + intersections[3].traversal1.edgeIndex + " " + intersections[3].traversal1.side.halfspace);
				//Debug.Log("out " + index_out + " in " + index_in);
				//Debug.Log("edge " + intersections[index_out].traversal1.edgeIndex + " to " + intersections[index_in].traversal1.edgeIndex);

				//float timeT2 = Time.realtimeSinceStartup;
			...
			//Debug.Log((time0 - time1) + " " + (time1 - time2) + " " + (time2 - time3) + "| " + timeA+ " " + timeB + " " + timeC + " " + timeD + " " + timeE);
			return true;
		}
...
//			var currVertexIndex		= controlMesh.GetVertexIndex(currEdgeIndex);
		...
		public static bool Triangulate(CSGBrush brush, ControlMesh controlMesh, Shape shape)
		{
			...
			var polygonPlanes = new List<CSGPlane>(controlMesh.Polygons.Length);
			for (var p = 0; p < controlMesh.Polygons.Length; p++)
			{
				var plane = GeometryUtility.CalcPolygonPlane(controlMesh, (short)p);
				polygonPlanes.Add(plane);
				shape.Surfaces[p].Plane = plane;
			}
			//float time1 = Time.realtimeSinceStartup;

			var polyTriangles	= new Dictionary<int, Triangle[]>(controlMesh.Polygons.Length);
			var newTriangles	= new List<Triangle>(100);
			...
			for (var p = controlMesh.Polygons.Length - 1; p >= 0; p--)
			{
				...
				var polygonPlane = polygonPlanes[p];
				var isPlanar = true;
				for (var e = 0; e< edgeIndices.Length; e++)
				{
				...
				for (var e = 0; e < edgeIndices.Length; e++)
				{
					...
					dataCache.HardEdges[pair] = polygonIndex;
				}

				var polyVerts = new short[edgeIndices.Length];
				for (var i = 0; i < polyVerts.Length; i++)
					polyVerts[i] = controlMesh.Edges[edgeIndices[i]].VertexIndex;

				short[] foundTriangles;
				SplitAtEdge(controlMesh, shape, out foundTriangles, dataCache, polyVerts, polyVerts.Length);
				...
				while (foundTriangles != null && foundTriangles.Length > 0)
				{
					...
					{
						newTriangles.Add(dataCache.AllTriangles[foundTriangles[0]]);
						break;
					}
					...
					{
						var triangle = dataCache.AllTriangles[foundTriangles[t]];
						...
						var hardEdgeCount = 0;
						if (dataCache.HardEdges.ContainsKey(vertPair1)) hardEdgeCount++;
						if (dataCache.HardEdges.ContainsKey(vertPair2)) hardEdgeCount++;
						if (dataCache.HardEdges.ContainsKey(vertPair3)) hardEdgeCount++;
					...
					{
						var triangle = dataCache.AllTriangles[foundTriangles[found]];
						var vertPair1 = VertPair.Create(triangle.VertexIndices[0], triangle.VertexIndices[1]);
						var vertPair2 = VertPair.Create(triangle.VertexIndices[1], triangle.VertexIndices[2]);
						var vertPair3 = VertPair.Create(triangle.VertexIndices[2], triangle.VertexIndices[0]);
						...
						if (!dataCache.HardEdges.ContainsKey(vertPair1)) dataCache.HardEdges.Add(vertPair1, -1);
						if (!dataCache.HardEdges.ContainsKey(vertPair2)) dataCache.HardEdges.Add(vertPair2, -1);
						if (!dataCache.HardEdges.ContainsKey(vertPair3)) dataCache.HardEdges.Add(vertPair3, -1);
					}
				}

				polyTriangles.Add(p, newTriangles.ToArray());
			}
			//float time2 = Time.realtimeSinceStartup;

			//var original_polygon_length = controlMesh.polygons.Length;
			var ownedPolygons = new HashSet<short>();
			foreach(var pair in polyTriangles)
			{
				...
				ownedPolygons.Clear();
				ownedPolygons.Add(polygonIndex);
				...
				{
					polygonPlanes.Add(GeometryUtility.CalcPolygonPlane(controlMesh, (short)i));
					ownedPolygons.Add((short)i);
				}
			}
			//float time3 = Time.realtimeSinceStartup;

			var polygonPlaneArray = polygonPlanes.ToArray();
			...
			*/
			...
			RemoveEdges(controlMesh, removeEdges); removeEdges.Clear();
			...
			//float time8 = Time.realtimeSinceStartup;

			// TODO: handle self-intersecting polygons


			CalculatePlanes(controlMesh, shape);
			//float time9 = Time.realtimeSinceStartup;

			Validate(controlMesh, shape);
			//float time10 = Time.realtimeSinceStartup;

			//Debug.Log((time0 - time1) + " " + (time1 - time2) + " " + (time2 - time3) + " " + (time3 - time4) + " " + (time4 - time5) + " " +
			//	(time5 - time6) + " " + (time7 - time8) + " " + (time8 - time9) + " " + (time9 - time10));
		...
		public static ControlMesh CreateFromShape(Shape shape, float distanceEpsilon = MathConstants.DistanceEpsilon)//, Vector3 const& translation)
		{
		//	var surfaces		= shape.surfaces;
			...
			// remove all coinciding and aligned planes
			for (var p1 = 0; p1 < surfaceCount - 1; p1++)
			{
				var plane1 = surfaces[p1].Plane;

				var planeSqrMagnitude = plane1.normal.sqrMagnitude;
				if (planeSqrMagnitude < MathConstants.EqualityEpsilonSqr ||
					float.IsNaN(plane1.a) || float.IsInfinity(plane1.a) ||
					float.IsNaN(plane1.b) || float.IsInfinity(plane1.b) ||
					float.IsNaN(plane1.c) || float.IsInfinity(plane1.c) ||
					float.IsNaN(plane1.d) || float.IsInfinity(plane1.d))
				{
					if (!removeList.Contains(p1)) removeList.Add(p1);
					continue;
				}

				if (planeSqrMagnitude > 1 + MathConstants.EqualityEpsilonSqr)
				{
					plane1.Normalize();
					surfaces[p1].Plane = plane1;					
				} 

				for (var p2 = p1 + 1; p2 < surfaceCount; p2++)
				{
					var plane2 = surfaces[p2].Plane;
					var alignment = plane1.Alignment(plane2);
					switch (alignment)
					{
						case PlaneAlignment.CoincidingFacingSameDirection:
						{
							if (!removeList.Contains(p1)) removeList.Add(p1);
							goto ExitSecondLoop;	// we have two planes that lie on top of each other, with both facing in the same direction ...
						}

						case PlaneAlignment.DisjointAndInFront:
						{
							if (!removeList.Contains(p1)) removeList.Add(p1);
							goto ExitSecondLoop;	// we have two planes that face in the same direction, with p1 in front of p2.
						}
						case PlaneAlignment.DisjointAndBehind:
						{
							if (!removeList.Contains(p2)) removeList.Add(p2);
							continue;				// we have two planes that face in the same direction, with p2 in front of p1.
						}

						case PlaneAlignment.Invalid:															// should never happen ...
						{
							Debug.LogWarning("Invalid plane in brush");
							return null;
						}
						case PlaneAlignment.DisjointFacingInwards:												// we have two planes that face inwards from each other
						case PlaneAlignment.CoincidingFacingOppositeDirection:	return null;					// we have two planes that lie on top of each other, with one facing in the opposite direction ...

						default:
						case PlaneAlignment.Intersecting:
						case PlaneAlignment.DisjointFacingOutwards:                                             // we have two planes that face outwards from each other
						{
							continue;
						}
					}
			...
			{
				int planeIndex = removeList[i];
				ArrayUtility.RemoveAt(ref shape.Surfaces, planeIndex);
				ArrayUtility.RemoveAt(ref shape.TexGens, planeIndex);
				ArrayUtility.RemoveAt(ref shape.TexGenFlags, planeIndex);
				ArrayUtility.RemoveAt(ref shape.Materials, planeIndex);
			}
			surfaces		= shape.Surfaces;
			surfaceCount	= shape.Surfaces.Length;
			//*/
...
//				var polygon_bounds	= polygonBounds[s];
		...
		public static bool RebuildShapeFrom(CSGBrush brush, ControlMesh srcControlMesh, Shape srcShape)
		{
            ...
            {
                triangulatedControlMesh.IsValid = false;
				return false;
			}
            ...
            brush.ControlMesh.IsValid	= true;
            return true;
		}
		...
		internal static bool UpdateBrushMesh(CSGBrush brush, ControlMesh controlMesh, Shape srcShape)
		{
			...
			{
				//brushCache.TriangulatedMesh = controlMesh;
				...
				if (brushCache.childData != null &&
					brushCache.childData.Model != null)
				{
					updateModels.Add(brushCache.childData.Model);
				}
		...
		public static void TransformBrushes(CSGBrush[] brushes, Vector3 centerBefore, Vector3 centerAfter, Vector3 scale, SpaceMatrices spaceMatrices)
		{
			...
			for (int t = 0; t < brushes.Length; t++)
			{
				...
				if (invertBrush)
				{
					...
					InvertShape(shape);
					controlMesh.IsValid = Validate(controlMesh, shape);
				}
				CalculatePlanes(controlMesh, shape);
				brush.EnsureInitialized();
		...
		public static bool DeleteSelectedPoints(CSGBrush[] brushes, ControlMeshState[] controlMeshStates)
		{
			...
			for (var t = brushes.Length - 1; t >= 0; t--)
			{
				...
				{
					Undo.DestroyObjectImmediate(brushes[t]);
