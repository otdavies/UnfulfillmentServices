...
namespace AmplifyShaderEditor
{
	...
	public sealed class RotatorNode : ParentNode
	{
		...
		public override string GenerateShaderForOutput( int outputId, ref MasterNodeDataCollector dataCollector, bool ignoreLocalvar )
		{
			...
			dataCollector.AddLocalVariable( m_uniqueId, "float " + cosVar + " = cos( "+time+" );");
			dataCollector.AddLocalVariable( m_uniqueId, "float " + sinVar + " = sin( "+time+" );");


			string rotatorVar = "rotator" + m_uniqueId;
			dataCollector.AddLocalVariable( m_uniqueId, "float2 " + rotatorVar + " = mul(" + result + " - "+anchor+", float2x2("+cosVar+",-"+sinVar+","+sinVar+","+cosVar+")) + "+anchor+";" );
