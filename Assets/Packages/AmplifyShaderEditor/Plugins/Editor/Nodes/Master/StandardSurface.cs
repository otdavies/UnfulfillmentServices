...
namespace AmplifyShaderEditor
{
	...
	{
		private readonly string[] VertexLitFunc = { "\t\tinline fixed4 LightingUnlit( SurfaceOutput s, half3 lightDir, half atten )",
													"\t\t{",
		...
		private GUIContent ShaderLODContent = new GUIContent( "LOD", "Shader LOD" );
		...
		public void DrawMaterialInputs( GUIStyle toolbarstyle )
		{
			...
			UIUtils.CurrentWindow.ExpandedProperties = GUILayoutToggle( UIUtils.CurrentWindow.ExpandedProperties, PropertyOderFoldoutStr, m_inspectorFoldoutStyle );
		...
		public override void DrawProperties()
		{
			...
			EditorGUILayout.BeginVertical();
			{
				...
				m_tessOpHelper.Draw( this, m_inspectorToolbarStyle, m_currentMaterial, m_inputPorts[ m_tessOpHelper.MasterNodeIndexPort ].IsConnected );
				...
				for ( int i = 0; i < sortedPorts.Count; i++ )
				{
					if ( sortedPorts[ i ].IsConnected )
					{
						...
						else if ( sortedPorts[ i ].DataName.Equals( VertexDataStr ) )
						{
							...
							UIUtils.CurrentDataCollector.AddToVertexDisplacement( vertexInstructions, m_vertexMode );
						}
