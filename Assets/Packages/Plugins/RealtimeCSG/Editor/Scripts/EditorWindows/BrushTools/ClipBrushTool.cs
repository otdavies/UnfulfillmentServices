...
namespace RealtimeCSG
{
	...
	[Serializable]
	internal sealed class ClipBrushTool : ScriptableObject, IBrushTool
	{
		...
		void UpdateMouseIntersection()
		{
			...
			} else
			{
				...
				if (pointsUsed > 0 && camera != null && assume2DView)
				{
					...
					movePlane = RealtimeCSG.Grid.CurrentGridPlane;					
			...
			if (currentMousePoint.HasValue)
			{
				...
				currentMousePoint = CSGBrushEditorManager.SnapPointToGrid(currentMousePoint.Value, movePlane, ref visualSnappedLines, out snapOnBrush, null);
				if (snapOnBrush != null)
...
//		MouseCursor lastUsedCursor = MouseCursor.Arrow;
		...
		void CreateFirstPoint()
		{
			...
			if (currentOnBrush != null)
			{
					...
					brush_cache.childData.ModelTransform != null)
				{
					...
					if (SceneQueryUtility.FindBrushIntersection(currentOnBrush, brush_cache.childData.ModelTransform.position, ray_start, ray_end, out intersectionFar, forceUseInvisible: true))
					{
			...
			if ((points[2] - points[1]).sqrMagnitude < 0.1f)
			{
				...
				var snapVector = RealtimeCSG.Grid.CurrentGridSnapVector;
		...
		{					
			...
			{
				Rect windowRect = new Rect(0, 0, currentSceneView.position.width, currentSceneView.position.height - GUIStyleUtility.BottomToolBarHeight);
...
//			onHandle = false;
				...
				case EventType.Repaint:
				{
					...
					{ 
						...
						//if (TryFindIntersection(world_ray, out intersectionPoint))
						{
							...
							float handleSize	= GUIStyleUtility.GetHandleSize(intersectionPoint);
				...
				case EventType.Repaint:
				{
					...
					{ 
						...
						{ 
							...
							{
								float handleSize	= GUIStyleUtility.GetHandleSize(points[i]);
		...
		public void HandleEditPointEvents(Rect sceneRect)
		{
			...
			for (int i = 0; i < 3; i++)
			{
				sizes[i] = GUIStyleUtility.GetHandleSize(points[i]) * ToolConstants.handleScale;
		...
		public void OnInspectorGUI(EditorWindow window, float height)
		{
			...
			{
				ClipToolGUI.OnInspectorGUI(window, height);
			}
		...
		{
			return ClipToolGUI.GetLastSceneGUIRect(this);
		}
		...
		public bool OnSceneGUI(Rect windowRect)
		{
			...
			ClipToolGUI.OnSceneGUI(windowRect, this);
			return true;
		}
	}
}
