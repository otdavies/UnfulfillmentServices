...
{	
	...
	{	
		...
		public void GenerateFromPolygon(CSGBrush brush, CSGPlane plane, Vector3 direction, Vector3[] meshVertices, int[] indices, uint[] smoothingGroups, bool drag)
		{
			...
			RealtimeCSG.Grid.SetForcedGrid(newPlane);
        ...
        internal override bool StartExtrudeMode(bool showErrorMessage = true)
		{
			...
			var usedSmoothingGroupIndices = SurfaceUtility.GetUsedSmoothingGroupIndices();
			var curvedEdgeTexgens	= new TexGen[outlineVertices.Length];
			var	curvedShapeEdges	= new ShapeEdge[outlineVertices.Length];
		...
		protected override void HandleCreateShapeEvents(Rect sceneRect)
		{
				...
				camera.pixelRect.Contains(current.mousePosition))
			{
				...
				base.visualSnappedGrid = RealtimeCSG.Grid.FindAllGridEdgesThatTouchPoint(worldPosition);
		...
		{			
			...
			// render edges
			if (!SceneTools.IsDraggingObjectInScene && settings.curveEdgeHandles != null &&
					...
					settings.curveTangentHandles.Length == settings.curve.Tangents.Length)
				{
					...
					for (int i = 0; i < settings.curveTangentHandles.Length; i+=2)
					{
						if (settings.curveTangentHandles[i + 0].Constraint != HandleConstraints.Straight ||
							settings.curveTangentHandles[i + 1].Constraint != HandleConstraints.Straight)
							curvedEdgeCount += 2;
					}
				...
				Event.current.type == EventType.Repaint)
			{
				...
				{
					float handleSize = GUIStyleUtility.GetHandleSize(curvedVertices[i]) * 0.5f;
				...
				{/*
					...
					case EventType.MouseDrag:
					{
							...
							(Tools.viewTool == ViewTool.None || Tools.viewTool == ViewTool.Pan))
						{
							...
							RealtimeCSG.Grid.SetForcedGrid(buildPlane);
							...
							if (snapFunction != null)
							{
								...
								for (int p = 0; p < settings.backupCurve.Points.Length; p++)
								{
									if (p == i)
										continue;
									float handleSize = GUIStyleUtility.GetHandleSize(settings.backupCurve.Points[p]);
					...
					switch (Event.current.GetTypeForControl(id))
					{
						...
						case EventType.MouseDrag:
						{
								...
								(Tools.viewTool == ViewTool.None || Tools.viewTool == ViewTool.Pan))
							{
								...
								var alignedPlane = new CSGPlane(RealtimeCSG.Grid.CurrentWorkGridPlane.normal, vertex);
								...
								visualSnappedGrid = RealtimeCSG.Grid.FindAllGridEdgesThatTouchPoint(difference + vertex);
				...
				switch (Event.current.GetTypeForControl(id))
				{
					...
					case EventType.MouseDrag:
					{
							...
							(Tools.viewTool == ViewTool.None || Tools.viewTool == ViewTool.Pan))
						{
							...
							var alignedPlane	= new CSGPlane(RealtimeCSG.Grid.CurrentWorkGridPlane.normal, settings.backupCurve.Points[i]);
							...
							visualSnappedGrid = RealtimeCSG.Grid.FindAllGridEdgesThatTouchPoint(difference + settings.backupCurve.Points[i]);
