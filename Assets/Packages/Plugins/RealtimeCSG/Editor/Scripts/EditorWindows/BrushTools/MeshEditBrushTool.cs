...
//			var brushLocalToWorld = targetMeshState.BrushTransform.localToWorldMatrix;
			...
			CSGBrushEditorManager.GenerateFromSurface(brush, polygonPlane, worldNormal, points, pointIndices, smoothingGroups, drag);
		}
		...
		private void UpdateGrid(Camera camera)
		{
				...
				(SelectionUtility.CurrentModifiers & EventModifiers.Control) != EventModifiers.Control)
			{
				...
				RealtimeCSG.Grid.SetupRayWorkPlane(worldOrigin, worldDirection, ref _movePlane);
				...
				(SelectionUtility.CurrentModifiers & EventModifiers.Control) != EventModifiers.Control)
			{
				...
				if (Tools.pivotRotation == PivotRotation.Global)
					worldDirection = GeometryUtility.SnapToClosestAxis(worldDirection);
				RealtimeCSG.Grid.SetupWorkPlane(worldOrigin, worldDirection, ref _movePlane);
			...
			{ 	
				RealtimeCSG.Grid.SetupWorkPlane(_originalPoint, ref _movePlane);
		...
		void UpdateChamferIntersectionPoint()
		{
			...
			for (int p0 = 0; p0 < chamferPlanes.Length; p0++)
			{
				...
				RealtimeCSG.Grid.SetForcedGrid(chamferPlanes[p0]);
			...
			switch (chamferMode)
			{
				...
				case ChamferMode.Point:
				{
					...
					if (doSnapping)
					{
						chamferIntersectionPoint = intersection + RealtimeCSG.Grid.SnapDeltaToGrid(intersection - projectedPoint, new Vector3[] { intersection });//, snapToSelf: true);
					} else
						chamferIntersectionPoint = intersection + RealtimeCSG.Grid.HandleLockedAxi(intersection - projectedPoint);
					break;
				}
		...
		void HandleChamfering()
		{
			...
			switch (type)
			{
				...
				case EventType.Repaint:
				{
					...
					PaintUtility.SquareDotCap(-1, chamferIntersectionPoint, MathConstants.identityQuaternion, GUIStyleUtility.GetHandleSize(chamferIntersectionPoint) * ToolConstants.handleScale);
					break; 
				}
			}
		}
		...
		public void HandleEvents(Rect sceneRect)
		{
			...
			if (currentHotControl != _chamferId &&
				!SceneTools.IsDraggingObjectInScene &&
								...
								switch (type)
								{
									...
									case EventType.MouseDrag:
									{
										...
										if (doSnapping)
										{
											...
											_worldDeltaMovement = RealtimeCSG.Grid.SnapDeltaToGrid(_worldDeltaMovement, worldPoints, snapToSelf: true);
										} else
										{
											_worldDeltaMovement = RealtimeCSG.Grid.HandleLockedAxi(_worldDeltaMovement);
										}
									...
									{					
										...
										if (doSnapping)
										{
											...
											if (_movePlaneInNormalDirection && _editMode != EditMode.ScalePolygon)
											{
												...
												_worldDeltaMovement = RealtimeCSG.Grid.SnapDeltaToRay(new Ray(worldLineOrg, worldLineDir), _worldDeltaMovement, worldPoints);
											} else
											{
												//for (int i = 0; i < worldPoints.Length; i++)
												//	worldPoints[i] = GeometryUtility.ProjectPointOnPlane(movePlane, worldPoints[i]);// - center));
		...
		public void OnInspectorGUI(EditorWindow window, float height)
		{
			...
			{
				MeshToolGUI.OnInspectorGUI(window, height);
			}
		...
		public bool OnSceneGUI(Rect windowRect)
		{
			...
			MeshToolGUI.OnSceneGUI(windowRect, this);
			return true;
		}
	}
}
