...
{
	[Serializable]
    internal sealed class LinearStairsGenerator : GeneratorBase, IBrushGenerator
	{
		...
		[SerializeField] LinearStairsSettings settings = new LinearStairsSettings();
		...
		{
			ResetVisuals();
			RealtimeCSG.Grid.SetForcedGrid(new CSGPlane(fromGridQuaternion * Vector3.up, settings.bounds.Min));
			switch (editMode)
			{
				default:
				{
		...
		public bool OnShowGUI(bool isSceneGUI) { return LinearStairsGeneratorGUI.OnShowGUI(this, isSceneGUI);	}
