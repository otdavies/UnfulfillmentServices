...
{
	internal sealed class ObjectEditBrushTool : ScriptableObject, IBrushTool
	{
		...
		void UpdateTargetBoundsHandles()
		{
			...
			//if (!Camera.current.orthographic)
			{
				...
				for (int i = renderLocalCenterPoints.Length - 1; i >= 0; i--)
				{
					...
					var handleSize = GUIStyleUtility.GetHandleSize(renderLocalCenterPoints[i]);
					if (backfaced[i])
					{
						if (!ortho)
						{
				...
				for (int i = renderLocalEdgePoints.Length - 1; i >= 0; i--)
				{
					...
					var handleSize = GUIStyleUtility.GetHandleSize(renderLocalEdgePoints[i]);
		...
		public void MoveCenter(Vector3 oldCenter, Vector3 newCenter, bool registerUndo = true)
		{
				...
				!float.IsInfinity(difference.z))
			{
				if (difference != Vector3.zero)
				{
					...
					{
						haveCloned = true;
						topTransforms = CSGBrushEditorManager.CloneTargets(delegate (Transform newTransform, Transform originalTransform)
						{
		...
		void UpdateGrid(Camera camera)
		{
			...
			if (hoverOnBoundsCenter != -1)
			{
				...
				RealtimeCSG.Grid.SetupRayWorkPlane(worldOrigin, worldDirection, ref movePlane);
			} else
			{
				RealtimeCSG.Grid.SetupWorkPlane(originalPoint, ref movePlane);
			}
		}
		...
		bool MoveObjects(Transform[] transforms, Vector3 offset, out Vector3 snappedOffset, bool doSnapping)
		{
			...
			if (doSnapping)
			{
				...
				offset = RealtimeCSG.Grid.SnapDeltaToGrid(offset, localPoints, snapToSelf: true);
			} else
			{
				offset = RealtimeCSG.Grid.HandleLockedAxi(offset);
			}
				...
				cloneDragKeyPressed)
			{
				if (toolEditMode == ToolEditMode.MovingObject)
				{
					...
					transforms = CSGBrushEditorManager.CloneTargets(delegate(Transform newTransform, Transform originalTransform) 
					{
						newTransform.localScale		= originalTransform.localScale;
		...
		internal void CloneMoveByOffset(Vector3 offset)
		{
			...
			topTransforms = CSGBrushEditorManager.CloneTargets();
			MoveByOffset(offset);
		...
		void MoveBoundsCenter(int boundsCenterIndex, Vector3 offset, out Vector3 snappedOffset, bool doSnapping)
		{
			...
			if (doSnapping)
			{
				...
				worldOffset			= RealtimeCSG.Grid.SnapDeltaToRay(new Ray(worldLineOrg, worldLineDir), worldOffset, worldPoints, snapToSelf: true);
		...
		void MoveBoundsEdge(int edgeCenter, Vector3 offset, out Vector3 snappedOffset, bool doSnapping)
		{
			...
			if (doSnapping)
			{
				...
				worldOffset = RealtimeCSG.Grid.SnapDeltaToGrid(worldOffset, worldPoints, snapToSelf: true);
				localOffset = activeSpaceMatrices.activeWorldToLocal.MultiplyVector(worldOffset);
			} else
			{
				worldOffset = RealtimeCSG.Grid.HandleLockedAxi(worldOffset);
				localOffset = activeSpaceMatrices.activeWorldToLocal.MultiplyVector(worldOffset);
			}
			if (float.IsNaN(localOffset.x) ||
				float.IsNaN(localOffset.y) ||
		...
		void UpdateRotationCircle(Matrix4x4 localToWorld, Vector2 mousePosition)
		{
			...
			float handleSize	= GUIStyleUtility.GetHandleSize(rotateCenter);
						...
						if (!Camera.current.orthographic && CSGSettings.GridVisible)
						{
							...
							if (!mouseIsDragging || (Tools.current != Tool.Rotate))
							{
								for (int i = 0; i < indices.Length; i++)
								{
									...
									var pt1 = RealtimeCSG.Grid.CurrentGridPlane.Project(pt0);
								...
								GUIUtility.hotControl == 0)
							{
								...
								if (activeTransform != null)
								{
									...
									for (int t = 0; t < brushes.Length; t++)
									{
										...
										if (SceneQueryUtility.FindBrushIntersection(brush, modelTranslation, ray_start, ray_end, out intersection, forceUseInvisible: true))
										{
												...
												for (int t = 0; t < targetBoundEdgeIDs.Length; t++)
												{
													if (targetBoundEdgeIDs[t] == nearestControl)
													{
															...
															renderLocalEdgePointSizes[t] > 0)
														{
															...
															if (Tools.current == Tool.Scale ||
																Tools.current == Tool.Rect)
																RealtimeCSG.Grid.SetForcedGrid(movePlane);
					...
					switch (type)
					{
						...
						case EventType.Repaint:
						{
							...
							if (disabled)
							{
								switch (BoundsUtilities.AABBSideAxis[hoverOnBoundsCenter])
								{
									case PrincipleAxis.X: PaintUtility.DrawScreenText(textCenter2D, "X axis is locked", GUIStyleUtility.redTextArea); break;
									case PrincipleAxis.Y: PaintUtility.DrawScreenText(textCenter2D, "Y axis is locked", GUIStyleUtility.redTextArea); break;
									case PrincipleAxis.Z: PaintUtility.DrawScreenText(textCenter2D, "Z axis is locked", GUIStyleUtility.redTextArea); break;
								}
						...
						case EventType.MouseDrag:
						{
							...
							if (!float.IsNaN(worldIntersection.x) && !float.IsNaN(worldIntersection.y) && !float.IsNaN(worldIntersection.z))
							{
								if (((prevModifiers & EventModifiers.Shift) != (SelectionUtility.CurrentModifiers & EventModifiers.Shift) 
									...
									RealtimeCSG.Grid.ForceGrid && startCamera != null)
								{
		...
		public void OnInspectorGUI(EditorWindow window, float height)
		{
			...
			{
				ObjectToolGUI.OnInspectorGUI(window, height);
			}
		...
		{
			return ObjectToolGUI.GetLastSceneGUIRect(this);
		}
		...
		public bool OnSceneGUI(Rect windowRect)
		{
			...
			ObjectToolGUI.OnSceneGUI(windowRect, this);
			return false;
		}
	}
}